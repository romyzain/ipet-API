...
module.exports = {
	...
	forgetPassword: async (req, res) => {
		try {
			...
			if(user.length === 0){
                throw new Error('No account with such email')
            }
            let token = createJWTToken({ ...user[0] })
			res.status(200).send({
				status: 'success',
				data: user,
				message: 'success',
